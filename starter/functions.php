...
function starter_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Footer Top', 'starter' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Footer Bootom', 'starter' ),
...
function starter_scripts() {
	wp_enqueue_style( 'starter-style_lato', "http://fonts.googleapis.com/css?family=Lato:400,700" );
	wp_enqueue_style( 'starter-style_customize', get_stylesheet_uri() );
	wp_enqueue_style( 'starter-style-font-awesome', get_template_directory_uri() . '/css/font-awesome.css' );
	wp_enqueue_style( 'starter-style', get_template_directory_uri() . '/css/smarter.css' );
...
/// get all latest post 
function load_latestblog(){ 

     $posts = get_posts(array("post_type" => "post",'posts_per_page' => 3)); 
	 echo "<h2 class='titleclass'>Latest Blog</h2>";
	 foreach($posts as $post){
		 echo "<div class='blogpost-wrapper'>";
        //getting thumb image
        $url_thumb = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'medium', false );
        $link = get_permalink($post); 
        echo "<div class='blogmain'>";
            if ( ! empty( $url_thumb ) ) { echo "<img src=".$url_thumb[0]." />";} 
			if ( ! empty( $post->post_content ) ) { echo "<div class='hidecontent'><p>".substr($post->post_content,0,160)."</p>";}
			if ( ! empty( $link ) ) { echo "<a href=".$link.">Read more</a></div>"; }
		echo "</div>";
        echo "<h2>".$post->post_title." </h2>";
        if ( ! empty( $post->post_content ) ) {  echo"<p>". substr($post->post_content,0,50)."</p>"; }
       echo "</div>";
    }
}
add_shortcode("latestblog","load_latestblog");
